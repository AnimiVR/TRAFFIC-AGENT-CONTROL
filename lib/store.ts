...
import type { Agent, AppState, ProjectMetrics, WorkItem, WalletState } from './types';
...
export interface UIState extends AppState {
  ...
  // Wallet reducers
  setWalletConnected: (connected: boolean, publicKey?: string, walletName?: string) => void;
...
export function createAppStore(initial?: Partial<AppState>) {
  return createStore<UIState>()((set, get) => ({
    ...
    wallet: initial?.wallet ?? emptyWallet,
    lastTickId: 0,
    ...
    applySnapshot(snapshot) {
      set({
        ...
        wallet: snapshot.wallet ?? emptyWallet,
    ...
    setWalletConnected(connected: boolean, publicKey?: string, walletName?: string) {
      set({
        wallet: {
